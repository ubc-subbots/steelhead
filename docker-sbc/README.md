...
## How to use
1. From the root of the repository, build the image by running `set -o pipefail; docker build -f docker-sbc/Dockerfile -t "steelhead-lyrical:<version>" . 2>&1 | tee build.log`. `<version>` convention is MAJOR.MINOR.BUGFIX-ARCH, e.g. `1.0.0-amd64`
2. Run the container with `sudo docker run -it --net=host -v /dev:/dev --privileged steelhead-lyrical:<version> bash`.
