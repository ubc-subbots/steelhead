...
class BoundingBoxImageSaver(Node):
    def __init__(self):
        ...
        self.subscribe_image = self.create_subscription(
            Image, "/steelhead/drivers/front_camera/image_raw", self.save_image, 10
        )
        self.subscriber_bbox = self.create_subscription(
            DetectionBoxArray,
            "/steelhead/gazebo_drivers/front_camera/bounding_box",
            self.save_bbox,
            10,
        )
        self.lastmsg = None
    ...
    def save_bbox(self, msg: DetectionBoxArray):
        if len(msg.boxes) == 0:
            return
        if (
            len(self.bbox_queue) > 100
        ):  # Keep last 100 bounding box msgs (bounding box is published at render time, image is published after modified)
    ...
    def save_image(self, msg: Image):
        ...
        try:
            if (
                self.lastmsg and bbox.x == self.lastmsg.x and bbox.y == self.lastmsg.y
            ):  # avoid duplicate images/bounding boxes
                return
            self.lastmsg = bbox
            ...
            data_dir = os.path.join(
                get_package_share_directory("steelhead_gazebo"), "data"
            )
