...
REPO_ROOT = subprocess.check_output(["git", "rev-parse", "--show-toplevel"]).decode(
    "utf-8"
)[:-1]
...
def convert_mesh(model_path, model):
    ...
    convert_mesh_path = os.path.join(
        REPO_ROOT, "src/steelhead_gazebo/scripts/import_mesh/convert_mesh.py"
    )
    run_cmd = [
        "blender",
        "--background",
...
def convert_urdf(model_path, model_name):
    ...
    if not os.path.exists(urdf_file):
        print(
            "ERROR: urdf file, %s not found, please ensure directory was generated correctly, exiting"
            % urdf_file
        )
        sys.exit(1)
    ...
    for key, data in parameters.items():
        ...
        if isinstance(data, dict):
            if key == "model" or key == "link":
                ...
                sdf_fmt.append(
                    indent + "<" + key + " name='%s_collision'>" % model_name
                )
            elif key == "shader":
            ...
            sdf_fmt.append(
                indent + "<" + key + ">" + " ".join(map(str, data)) + "</" + key + ">"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Creates a Gazebo model using a .STL file and a .urdf description."
    )
    parser.add_argument(
        "--model",
        "-m",
    ...
    with open(
        os.path.join(REPO_ROOT, "src/steelhead_gazebo/scripts/", args.parameters), "r"
    ) as param_file:
    ...
    result = "<?xml version='1.0'?>\n" + XMLCombiner(
        (
            os.path.join(args.model, "temp", "model_org.sdf"),
