...
def generate_launch_description():
    ...
    # The slalom model sits at z=-1.25 in this world, so holding 1.25m keeps the AUV
    # level with the middle of the poles for the depth bonus.
    hover_script = Node(
        package="steelhead_controls",
        executable="hover_at_depth",
        parameters=[{"depth": 1.25, "hold_yaw": True}],
        namespace="steelhead",
    )
    ...
    automation = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                ...
                "launch",
                "automated_competition_slalom_launch.py",
            )
        )
    )
