...
namespace steelhead_tasks
{
     ...
     * Wrench on the hover_at_depth adjustment topic (force.x/y nudge position,
     * force.z sign nudges depth, torque.z sign nudges yaw). When the gate has
    ...
    private:
        ...
        // forward drive command applied while approaching the gate
        double forward_drive_;
        // yaw command used to spin in place and search when the target has
        // never been seen; hover_at_depth only uses the sign
        double search_yaw_;
