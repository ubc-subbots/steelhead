...
FORWARD = Wrench(
    force=Vector3(x=DEFAULT_EFFORT, y=0.0, z=0.0), torque=Vector3(x=0.0, y=0.0, z=0.0)
)
BACKWARD = Wrench(
    force=Vector3(x=-DEFAULT_EFFORT, y=0.0, z=0.0), torque=Vector3(x=0.0, y=0.0, z=0.0)
)
RIGHT_TURN = Wrench(
    force=Vector3(x=0.0, y=0.0, z=0.0), torque=Vector3(x=0.0, y=0.0, z=-DEFAULT_EFFORT)
)
LEFT_TURN = Wrench(
    force=Vector3(x=0.0, y=0.0, z=0.0), torque=Vector3(x=0.0, y=0.0, z=DEFAULT_EFFORT)
)
RIGHT_SWAY = Wrench(
    force=Vector3(x=0.0, y=-DEFAULT_EFFORT, z=0.0), torque=Vector3(x=0.0, y=0.0, z=0.0)
)
LEFT_SWAY = Wrench(
    force=Vector3(x=0.0, y=DEFAULT_EFFORT, z=0.0), torque=Vector3(x=0.0, y=0.0, z=0.0)
)
UPWARD = Wrench(
    force=Vector3(x=0.0, y=0.0, z=DEFAULT_EFFORT), torque=Vector3(x=0.0, y=0.0, z=0.0)
)
DOWNWARD = Wrench(
    force=Vector3(x=0.0, y=0.0, z=-DEFAULT_EFFORT), torque=Vector3(x=0.0, y=0.0, z=0.0)
)
...
class PredeterminedThrust(Node):
    ...
    def callback(self):
        ...
        move_on = self.get_clock().now() - self.last_time > rclpy.time.Duration(
            seconds=thrusts[self.current_index]["duration"]
        )
        if move_on:
