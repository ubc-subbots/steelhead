...
void pubOdometry(const Estimator &estimator, const std_msgs::msg::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        odometry.pose.covariance[0] = 0.001;  // x
        odometry.pose.covariance[7] = 0.001;  // y
        odometry.pose.covariance[14] = 0.001; // z
        odometry.pose.covariance[21] = 0.01;  // roll
        odometry.pose.covariance[28] = 0.01;  // pitch
        odometry.pose.covariance[35] = 0.01;  // yaw

        // Twist covariance (vx, vy, vz, vroll, vpitch, vyaw)
        odometry.twist.covariance[0] = 0.001;  // vx
        odometry.twist.covariance[7] = 0.001;  // vy
        odometry.twist.covariance[14] = 0.001; // vz
        odometry.twist.covariance[21] = 0.01;  // vroll
        odometry.twist.covariance[28] = 0.01;  // vpitch
        odometry.twist.covariance[35] = 0.01;  // vyaw
...
void pubTF(const Estimator &estimator, const std_msgs::msg::Header &header)
{
    ...
    transform_cam.header.frame_id = "vins_frame";
    transform_cam.child_frame_id = "camera_link";
